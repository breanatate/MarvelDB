...
#
DATABASEURI = "postgresql://brt2109:7475@104.196.135.151/proj1part2"
...
def profile(alias):
  ...
  for person in cursor:
    ...
    results.append(row)
  cursor.close()
  cursor = g.conn.execute('SELECT b.affiliation FROM Belongs_To AS b AS a WHERE b.alias = (%s)' , alias)
  results1 = []
  for result in cursor:
    affiliation = result[0]
    cursor = g.conn.execute('SELECT i.alias FROM Belongs_To AS b AS a WHERE b.affiliation = (%s)' , affiliation)
    for result in cursor:
...
def results():
  ...
  for person in in_city:
    cursor = g.conn.execute('SELECT i.alias, i.name, i.appdate, i.uid FROM Individuals AS i WHERE i.alias = (%s)', person)
    for person in cursor:
      row = []
      row.append(person[0])
      row.append(person[1])
