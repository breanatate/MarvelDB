...
def results():
  ...
  cursor = g.conn.execute("SELECT Individuals.name, Individuals.uid FROM Individuals LEFT JOIN Universes ON Individuals.uid = Universes.uid WHERE Individuals.uid = 'tuid'")
  results = []
  for result in cursor:
